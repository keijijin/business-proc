...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "business-proc-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name business-proc-service apps/business-proc-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app business-proc-service:1.0-SNAPSHOT
    oc expose svc/business-proc-service
else

	echo "Launching the application locally..."
	pattern="business-proc-service"
	files=( $pattern )
	cd ${files[0]}
